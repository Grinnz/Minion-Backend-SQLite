...
subtest 'Migrate up and down' => sub {
  ...
  is $minion->backend->sqlite->migrations->migrate->active, 8, 'active version is 8';
};
...
subtest 'Repair missing worker' => sub {
  ...
  $minion->backend->sqlite->db->query(
    q{update minion_workers set notified = datetime('now', '-' || ? || ' seconds') where id = ?},
...
subtest 'Repair old jobs' => sub {
  ...
  my $finished = $minion->backend->sqlite->db->query(
    q{select strftime('%s',finished) as finished from minion_jobs where id = ?}, $id2
  )->hash->{finished};
  $minion->backend->sqlite->db->query(
  ...
  $finished = $minion->backend->sqlite->db->query(
    q{select strftime('%s',finished) as finished from minion_jobs where id = ?}, $id3
  )->hash->{finished};
  $minion->backend->sqlite->db->query(
    q{update minion_jobs set finished = datetime(?,'unixepoch') where id = ?},
...
subtest 'List locks' => sub {
  ...
  $minion->backend->sqlite->db->query(
    q{update minion_locks set expires = datetime('now', '-1 second') where name = 'yada'},
  );
...
subtest 'Reset (locks)' => sub {
  ...
  $minion->worker->register unless HAS_PSEUDOFORK;
  ok $minion->backend->list_jobs(0, 1)->{total},    'jobs';
  ok $minion->backend->list_locks(0, 1)->{total},   'locks';
  SKIP: {
    skip 'Minion workers do not support fork emulation', 1 if HAS_PSEUDOFORK;
...
subtest 'List jobs' => sub {
  ...
  SKIP: {
    skip 'Minion workers do not support fork emulation', 2 if HAS_PSEUDOFORK;
  ...
  TODO: {
    todo_skip 'filtering jobs by notes not yet implemented', 3;
    my $id2   = $minion->enqueue('test' => [] => {notes => {is_test => 1}});
    my $batch = $minion->backend->list_jobs(0, 10, {notes => ['is_test']})->{jobs};
